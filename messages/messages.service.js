...
  });
  
  // ดึงข้อมูลใหม่ที่มีการ include ครบถ้วน เพื่อส่งให้ shapeRow
...
  // ดึงข้อมูลข้อความ พร้อมกับ ChatRoom และ User ที่เกี่ยวข้องทั้งหมด
...
  await msg.save();
  
  // ส่งข้อมูลที่อัปเดตแล้วไปให้ shapeRow โดยตรง
  return shapeRow(msg);
}
...
  if (![msg.ChatRoom.headuserid, msg.ChatRoom.targetuserid].includes(me))
    throw { statusCode: 403, message: 'Forbidden: not a room member' };
  return msg; // คืนค่า Sequelize Instance ตรงๆ
}
